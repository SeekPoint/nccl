...
/*
 * 从根节点递归遍历下去，直到遇到nvlink xml节点，然后拿到nvlink的父节点，
 即gpu节点，然后通过tclass获取对端PCI设备类型，
 * 如果是gpu或者cpu，直接返回对端node，
 * 如果是nvswitch，那就先创建nvswitch节点，然后创建当前gpu节点和对端的双向边。
 然后通过ncclTopoConnectCpus将cpu两两连接。
 */
...
ncclResult_t ncclTopoGetSystem(struct ncclComm* comm, struct ncclTopoSystem** system) {
  ...
  // 然后遍历每个rank的hosthash，如果相等的话说明在同一个机器，
  // 然后执行ncclTopoFillGpu，将gpu加入到xml树
  for (int r=0; r<comm->nRanks; r++) {
    if (comm->peerInfo[r].hostHash == comm->peerInfo[comm->rank].hostHash) {
      char busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE];
      NCCLCHECK(int64ToBusId(comm->peerInfo[r].busId, busId));
      struct ncclXmlNode* node;
      //执行ncclTopoFillGpu，将gpu加入到xml树。
